...
/* really_really_load_png:
 *  Worker routine, used by load_png and load_memory_png.
 */
static BITMAP *really_really_load_png(png_structp png_ptr, png_infop info_ptr, RGB *pal)
{
    BITMAP *bmp;
    PALETTE tmppal;
	...
	if(png_get_PLTE(png_ptr, info_ptr, &palette, &num_palette)) {
		...
		pal[i].r = palette[i].red;	/* Simion32:  DO NOT CONVERT 256 TO 64 */
 ...
 */
BITMAP *really_load_png(PACKFILE *fp, RGB *pal)
{
    BITMAP *bmp;
    png_structp png_ptr;
    png_infop info_ptr;
    ...
    bmp = really_really_load_png(png_ptr, info_ptr, pal);
