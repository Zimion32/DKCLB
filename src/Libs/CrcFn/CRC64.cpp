...
**===========================================================================*/
#include "include.h"
static U64 const table_crc64[0x100] = {
...
{
	U64 r = 0xFFFFFFFFFFFFFFFFLL;
	for(U32 i = 0;i != size;i++,data++)
	{r = (r>>8) ^ table_crc64[(data[0] ^ r) & 0xFF];}
	return (~r);
}
U64 CRC64(Vx<U08>& data)
{
	return CRC64((U08*)&data[0], data.size());
}
U64 CRC64(TXT data)
{
	if(!data.size()) return 0x0000000000000000LL;
	return CRC64((U08*)&data[0], data.size());
}
